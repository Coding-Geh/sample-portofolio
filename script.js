...
// Mobile drawer
(function(){const btn=document.getElementById('menuToggle');const d=document.getElementById('navDrawer');const bd=document.getElementById('backdrop');const c=document.getElementById('drawerClose');if(!btn||!d||!bd)return;const firstLink=()=>d.querySelector('a');const open=()=>{d.classList.add('show');bd.classList.add('show');bd.hidden=false;btn.setAttribute('aria-expanded','true');document.body.style.overflow='hidden';const fl=firstLink();fl&&fl.focus({preventScroll:true});};const close=()=>{d.classList.remove('show');bd.classList.remove('show');btn.setAttribute('aria-expanded','false');document.body.style.overflow='';setTimeout(()=>{bd.hidden=true},250);btn.focus({preventScroll:true});};btn.addEventListener('click',()=>d.classList.contains('show')?close():open());bd.addEventListener('click',close);if(c) c.addEventListener('click',close);window.addEventListener('keydown',(e)=>{if(e.key==='Escape') close();});})();
